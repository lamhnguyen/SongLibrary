...
server.get("/songs/:slug", function (req, res) {
  const db = router.db;
  let song = getBy(db, "songs", "slug", req.params.slug);
  ...
  if (song) {
    ...
    result.genre = getById(db, "genres", song.genreId);
    result.key = getById(db, "keys", song.keyId);
    result.authors = getByIds(db, "authors", song.authorIds);
    result.poets = getByIds(db, "poets", song.poetIds);
    result.artists = getByIds(db, "artists", song.artistIds);
...
// Helpers
const slugify = (text) =>
  _.kebabCase(removeDiacritics(text.replace(/&/g, "-and-")));
...
function slugifySong(s) {
  return (
    ...
    (s.artists.length > 0
      ...
      : "") +
    "-LYRICS-" +
    slugify(s.lyrics.replace(/ *\[[^\]]*]/g, "")) // Remove chords (e.g. [Am]) from the lyrics
  );
}
